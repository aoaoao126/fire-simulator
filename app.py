...
with st.sidebar:
    ...
    with st.expander("📝 設定ガイド: リターン/リスク"):
        ...
        | 項目 | 推奨設定 | 説明 |
        | :--- | :--- | :--- |
        | **期待リターン** | 4% 〜 7% | S&P500の実質利回り目標。保守的なら4%が安全圏です。 |
        | **ボラティリティ** | 15% 〜 20% | 資産の上下幅。S&P500は**18%前後**です。大きくするほどリスクが高まります。 |
        | **インフレ率** | 2% | 中央銀行の目標値。資産の目減りを防ぐため必ず考慮すべき数値です。 |

        **📊 歴史データ（S&P 500）**
        - **-10%以上の調整**: 約1年に1回発生（一時的な押し目）
        - **-20%以上の暴落**: 約6.4年に1回発生（元の株価に戻るまで約2年）
        - **-30%以上の大暴落**: 約12年に1回発生（ITバブル崩壊・リーマン級）
        """)
    ...
    with st.expander("📉 設定ガイド: 暴落と現金保有"):
        st.markdown("""
        **💡 暴落判定の目安**
        市場ではピークから**20%下落すると「弱気相場（暴落）」**と定義されます。このラインを現金回避のスイッチにするのが一般的です。

        **🛡️ 3年ルールの重要性**
        歴史上、多くの暴落は**3年以内**に回復しています。**3年分の生活費（1,500万円前後）**を現金で持っておくことで、最悪の時期に資産を売らずにやり過ごせる確率が飛躍的に高まります。
        """)
