...
with st.sidebar:
    ...
    with st.expander("📝 設定ガイド: リターン/リスク"):
        ...
        | **ボラティリティ** | 15% 〜 20% | 資産の変動幅。S&P500は**18%前後**です。数字を大きくするほど激しい振れを体験できます。 |
    ...
    with st.expander("🛡️ 暴落への備え（現金3年ルール）"):
        st.markdown("""
        S&P 500の暴落は平均して**3年以内**に回復しています。

        このシミュレーターでは**3年分の生活費（1,500万円前後）**を現金で持っておく設定にすることで、暴落中に株を売らずにやり過ごす「生存戦略」を体験できます。
        """)
